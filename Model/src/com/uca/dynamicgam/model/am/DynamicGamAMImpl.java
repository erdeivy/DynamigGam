...
import com.uca.dynamicgam.model.vo.GamHistoryVORowImpl;
...
import com.uca.dynamicgam.model.vo.OperationsCountVOImpl;
import com.uca.dynamicgam.model.vo.OperationsCountVORowImpl;
...
public class DynamicGamAMImpl extends ApplicationModuleImpl implements DynamicGamAM {
    ...
    /**
     * Container's getter for GamHistory1.
     * @return GamHistory1
     */
    public ViewObjectImpl getGamHistoryVO() {
        return (ViewObjectImpl) findViewObject("GamHistoryVO");
    }
     ...
     * @return GamUsers1
     */
    public ViewObjectImpl getLoggedUserVO() {
        return (ViewObjectImpl) findViewObject("LoggedUserVO");
    }

    /**
     * Container's getter for SchemasByUser1.
     * @return SchemasByUser1
     */
    public ViewObjectImpl getSchemasByUserVO() {
        return (ViewObjectImpl) findViewObject("SchemasByUserVO");
    }

    /**
     * Container's getter for SchemasByUser_TablesByUser_VL1.
     * @return SchemasByUser_TablesByUser_VL1
     */
    ...
    /**
     * Container's getter for OperationsCount1.
     * @return OperationsCount1
     */
    public OperationsCountVOImpl getOperationsCountVO() {
        return (OperationsCountVOImpl) findViewObject("OperationsCountVO");
    }
    
    public void buildOperationsCountModel(){
        RowSetIterator rsi = getGamHistoryVO().createRowSetIterator(null);
        Map<String, Long> opers = new HashMap<String, Long>();
        while(rsi.hasNext()){
            GamHistoryVORowImpl history = (GamHistoryVORowImpl) rsi.next();
            Long count = opers.get(history.getOperation());
            if(count == null){
                count = new Long(0);
            }
            opers.put(history.getOperation(), new Long(count.longValue()+1));
        }
        rsi.closeRowSetIterator();

        getOperationsCountVO().executeEmptyRowSet();
        for (Map.Entry<String, Long> entry : opers.entrySet()) {
            OperationsCountVORowImpl operation = (OperationsCountVORowImpl) getOperationsCountVO().createRow();
            operation.setOperation(entry.getKey());
            operation.setCount(entry.getValue());
            getOperationsCountVO().insertRow(operation);
        }
    }
}
